...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv firstapp` or `python -m venv firstapp-venv`
...
- [ ] Make sure the virtualenv is activated `workon firstapp` or `source firstapp-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `firstapp/backend/firstapp/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `firstapp/backend/firstapp/settings/production.py`
...
- Create a copy of ``firstapp/settings/local.py.example``:  
  `cp firstapp/settings/local.py.example firstapp/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/firstapp/settings/base.py``.
- Adjust ``backend/firstapp/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
